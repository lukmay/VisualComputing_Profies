...
void main(void)
{
    ...
    float attenuation1 = 1.0f / (0.8f + 0.2f * distance1 + 0.2f * (distance1 * distance1));

    float distance2 = length(uFrontLight2.position - tFragPos);
    float attenuation2 = 1.0f / (0.8f + 0.2f * distance2 + 0.2f * (distance2 * distance2));
    ...
    if(theta1 > uFrontLight1.cutOff){
        illumination += uFrontLight1.color * 0.4f * attenuation1 * uMaterial.diffuse;
    }

    if(theta2 > uFrontLight2.cutOff){
        illumination += uFrontLight2.color * 0.4f *
        attenuation2 * uMaterial.diffuse;
    }

...
void wave(int index, float dt)
{
    for (size_t waveFuncIndex = 0; waveFuncIndex < 3; waveFuncIndex++)
    {
        sScene.water.vertices[index].pos.y =
            sScene.waterSim.parameter[waveFuncIndex].amplitude* sin(
                    sScene.waterSim.parameter[waveFuncIndex].omega * 
...
void updateWater(float dt)
{
    sScene.waterSim.accumTime += dt;
        for (size_t i = 0; i < sScene.water.vertices.size(); i++)
        {
            ...
            //printf("y=%f,x=%f,z=%f,dt=%f\n",sScene.water.vertices.at(i).pos.y, sScene.water.vertices.at(i).pos.x, sScene.water.vertices.at(i).pos.z, sScene.waterSim.accumTime );
        }
